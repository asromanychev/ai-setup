...
**Сценарии, которые должны быть покрыты (один it на сценарий, итого 19 новых examples):**
...
Цель сценария — доказать, что `Source#sync_step!` не зависит от конкретного класса plugin и одинаково работает с любым объектом, реализующим `#sync_step`. Этот инвариант **нельзя** материализовать через один и тот же тип `double`, так как doubles не различаются по реальному классу.
...
- Для этого сценария **не использовать** `double` / `instance_double`. Вместо этого объявить два минимальных анонимных класса внутри `it`-блока:
  ...
  ```
- Registry doubles для обоих классов настраиваются независимо: один `registry_a` возвращает `plugin_class_a.new`, другой `registry_b` — `plugin_class_b.new`.
- Verify: оба вызова `source.sync_step!` возвращают одинаковый Hash и обновляют checkpoint до `{ "cursor" => 1 }`.
- Для всех остальных сценариев (1–16, 18–19) продолжать использовать `double` / `instance_double`.
...
**Команда 5a — наблюдаемая проверка числа новых examples:**

До запуска полного suite убедиться, что в `describe "#sync_step!"` добавлено ровно 19 новых examples:
```
...
**Ожидаемый вывод команды 5a:** `19`. Если число меньше — шаг 4 не завершён, реализация не продолжается.
...
**Pass criteria:** exit 0, 0 failures, команда 5a = 19, pending совпадает со значением из шага 2.

**Если тест красный или команда 5a < 19:** шаг считается failed. Падение фиксируется как observational расхождение (failing output → отдельная задача Fix), реализация текущего плана на этом останавливается.
...
- `started_at`: 2026-04-06T23:00:00+03:00
- `finished_at`: 2026-04-06T23:05:00+03:00
- `elapsed_seconds`: 300
