...
| 20 | Все существующие тесты остаются зелёными (верифицируется в шаге 4) | AC 18 |
...
**Предварительная фиксация pre-existing pending (до начала шага 2):**
Перед реализацией (до шага 2) выполнить:
```
bundle exec rspec --format json | jq '[.examples[] | select(.status == "pending")] | length'
```
Если pre-existing pending count = 0, финальный gate строг: `0 pending` без исключений. Если pre-existing pending > 0, перечислить их явно здесь и исключить только их — любой новый pending из `describe "#sync_step!"` недопустим.

**Ожидаемый результат для текущего кода:** pre-existing pending = 0 (проверяется до начала реализации). Следовательно, финальный gate строг.
...
- [ ] **0 failures, 0 pending** — любой pending из `#sync_step!` тестов недопустим

**Pass criteria:** exit 0, 0 failures, 0 pending.
...
- `started_at`: 2026-04-06T20:00:00+03:00
- `finished_at`: 2026-04-06T20:05:00+03:00
- `elapsed_seconds`: 300
