...
8. Целевой файл: `spec/models/sync_checkpoint_spec.rb`
   Суть шага: создать model spec для `SyncCheckpoint`, покрывающий обязательность `source`, запрет `position: nil`, допустимость `{}`, поведение `source.sync_checkpoint` при 0/1 записях, каскадное удаление `source.destroy -> SyncCheckpoint.where(source_id: source.id).count == 0` как проверку `ON DELETE CASCADE` из миграции шага 3, и наблюдаемый trigger для неоднозначности при count > 1 через прямой SQL или иной обход AR-валидаций, если precondition из шага 1 закрыт.
