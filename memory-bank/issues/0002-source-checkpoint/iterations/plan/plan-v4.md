...
8. Целевой файл: `spec/models/sync_checkpoint_spec.rb`
   Суть шага: создать model spec для `SyncCheckpoint`, покрывающий обязательность `source`, запрет `position: nil`, допустимость `{}`, поведение `source.sync_checkpoint` при 0/1 записях и каскадное удаление `source.destroy -> SyncCheckpoint.where(source_id: source.id).count == 0` как проверку `ON DELETE CASCADE` из миграции шага 3; для ambiguity-сценария зафиксировать отдельный исполнимый setup: в примере временно снять unique index на `sync_checkpoints.source_id`, создать вторую строку с тем же `source_id` через прямой SQL/`insert_all!`, проверить `source.reload.sync_checkpoint` на `Collect::CheckpointAmbiguityError`, затем в `ensure` или эквивалентном teardown восстановить unique index, чтобы остальные примеры и финальная верификация работали на целевой схеме.
   Что уже есть в текущем коде: отдельного тестового файла и каталога `spec/models/` для checkpoint нет.
   Чего не хватает по `spec.md`: тестового подтверждения AC для `SyncCheckpoint`, каскадного удаления и сценария ambiguity с наблюдаемым способом материализации повреждённого состояния.
   Что в этом файле запрещено переписывать без отдельного наблюдаемого расхождения: контракт внешней ошибки `Collect::CheckpointAmbiguityError`; тестовый setup для ambiguity не должен оставлять схему без уникального индекса после завершения примера.
   Какие зависимости шаг разрешает: завершает покрытие acceptance criteria по checkpoint и создаёт наблюдаемый источник решений для точечных правок моделей без конфликта с постоянным DB-инвариантом шага 3.
