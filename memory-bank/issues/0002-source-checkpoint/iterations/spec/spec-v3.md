...
**НЕ входит:** логика оркестрации, фоновые джобы, хранение payload, валидация формата `position` (ответственность плагина #1), API-эндпоинты, провайдер-специфичные данные.
...
7. `position` — jsonb, not null; `nil` → `ActiveRecord::RecordInvalid`.
...
| `position: {}` (пустой Hash) | Валидация проходит; хранится как `{}` |
...
| `app/models/sync_checkpoint.rb` | AR: `belongs_to :source`, `validates :position, presence: true` |
| `core/lib/collect/errors.rb` | Добавить `Collect::CheckpointAmbiguityError < Collect::Error` |
| `config/application.rb` | `require 'collect'` — точка подключения `core/lib` к Rails autoload |
| `spec/rails_helper.rb` | Новый helper для Rails model specs (см. выше) |
