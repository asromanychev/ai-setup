...
**НЕ входит:** логика оркестрации, фоновые джобы, хранение payload, валидация формата `position` (ответственность плагина #1), API-эндпоинты, провайдер-специфичные данные, подключение `core/lib` к Rails runtime через `config/application.rb` (отдельный infrastructure issue), определение `Collect::CheckpointAmbiguityError` (добавляется в `core/lib` в отдельном infrastructure issue; предполагается существующим до начала реализации этого issue).

**Пре-условие:** `Collect::CheckpointAmbiguityError < Collect::Error` существует в `core/lib/collect/errors.rb` до начала реализации этого issue.
