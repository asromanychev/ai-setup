1. Паттерн оркестрации: один агент, последовательное выполнение.
   Порядок зависимостей: сначала точечная правка базового контракта в `core/lib/collect/plugin.rb`, затем реализация недостающего поведения в `core/lib/collect/null_plugin.rb`, после этого проверка `core/lib/collect/plugin_registry.rb` на необходимость изменений, и только затем расширение тестов по слоям `Plugin` -> `PluginRegistry` -> `NullPlugin` -> интеграционный сценарий реестра.
...
2. Grounding по кодовой базе: подтвердить перед изменениями, что файлы `core/lib/collect/plugin.rb`, `core/lib/collect/plugin_registry.rb`, `core/lib/collect/null_plugin.rb`, `spec/core/collect/plugin_contract_spec.rb`, `spec/core/collect/plugin_registry_spec.rb`, `spec/core/collect/null_plugin_spec.rb` и `spec/core/collect/registry_null_plugin_integration_spec.rb` существуют и соответствуют текущей архитектуре ядра `Collect`.
   Зафиксировать, что `plugin_registry.rb` уже реализует требования спецификации и должен изменяться только при обнаружении конкретного расхождения со `spec.md`, без рефакторинга "на всякий случай".

3. Целевой файл: `core/lib/collect/plugin.rb`
   Конкретное действие: добавить приватный рекурсивный helper `deep_freeze(value)` для `Hash`, `Array` и вложенных элементов; применить его к `source_config` после `deep_dup` в `initialize`; применить его к нормализованному результату `sync_step`, чтобы замораживались не только верхний `Hash`, но и все вложенные структуры в `:records` и `:checkpoint_out`.
...
5. Целевой файл: `core/lib/collect/plugin_registry.rb`
   Конкретное действие: выполнить проверку на соответствие `spec.md` и существующим acceptance criteria; если файл уже полностью соответствует требованиям по `register`, `fetch`, `build`, нормализации id и `default`, оставить его без изменений.
   Условие на правку: если обнаружится конкретное расхождение, исправить только это расхождение без изменения публичного поведения, уже покрытого спецификацией.

6. Целевой файл: `spec/core/collect/plugin_contract_spec.rb`
   Конкретное действие: расширить unit-тесты базового класса `Collect::Plugin`, чтобы они проверяли глубокую иммутабельность `source_config` и `checkpoint_in`, глубокую заморозку вложенных структур в результате `sync_step`, обязательность `.plugin_id` и `#perform_sync_step`, а также ошибки при неполном или неверно типизированном результате.
