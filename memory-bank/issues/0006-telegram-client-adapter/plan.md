...
**Date:** 2026-04-08  
**Changes from v1:** добавлен `retry_delay:` параметр в конструктор клиента для изолированных тестов retry; Шаг 3 дополнен примером использования.
...
module Telegram
  class ChannelClient
    ...
    def initialize(bot_token:, http_class: Net::HTTP, retry_delay: nil)
    ...
    def sleep_for(seconds)
      sleep(@retry_delay.nil? ? seconds : @retry_delay)
    end
  end
end
```
...
**Использование retry_delay: 0 в тестах (AC-6 и аналогичные):**
```ruby
let(:client) do
  Telegram::ChannelClient.new(
    bot_token: "test_token",
    http_class: stub_http_class([rate_limit_response, rate_limit_response, rate_limit_response]),
    retry_delay: 0   # ← избегает реальных sleep в unit tests
  )
end
```
Без `retry_delay: 0` тест AC-6 занимал бы ~2 секунды (2 sleep × 1 сек). С `retry_delay: 0` — мгновенно.
