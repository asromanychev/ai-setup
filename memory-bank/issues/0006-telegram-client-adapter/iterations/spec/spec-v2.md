...
**Date:** 2026-04-08  
**Changes from v1:** добавлены AC-15 (TimeoutError), AC-16 (stateless repeat call); инвариант 4 помечен как «enforced by code review».
...
4. **bot_token не логируется:** в Rails log не попадает значение `bot_token` ни при инициализации, ни при запросах. *(Enforced by code review; автоматический тест не добавляется — захват log output выходит за рамки unit-spec.)*
...
- [ ] **AC-15:** stub-транспорт симулирует `Net::OpenTimeout` → клиент поднимает `Telegram::TimeoutError`.
- [ ] **AC-16:** два последовательных вызова `fetch_posts` с одним и тем же `cursor` через stub-транспорт возвращают структурно идентичные результаты (клиент не сохраняет изменяемое состояние между вызовами).
...
| prompt_id | 03-2-fix-spec (wording fix + AC additions) |
